...
U, S, V = svd(kernel)
# vertical 1D filter
v = np.dot(np.sqrt(S[0]), U[:, 0])
# horizontal 1D filter
h = np.dot(np.sqrt(S[0]), V[0, :])

gflt_imns = conv1(imns, h, mode='reflect', axis=0)
gflt_imns = conv1(gflt_imns, v, mode='reflect')

gflt_imng = conv1(imng, h, mode='reflect', axis=0)
gflt_imng = conv1(gflt_imng, v, mode='reflect')
##--your-code-ends-here--##
...
def median_filter(img, wsize):
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            ##--your-code-starts-here--##
            median_pixel = np.median(img[iMin:iMax, jMin:jMax])
