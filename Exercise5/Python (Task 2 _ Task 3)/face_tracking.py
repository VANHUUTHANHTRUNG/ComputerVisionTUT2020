        ...
        if not ret_val:
            ...
            if len(p0) <= 10:
                ...
                if len(faces) != 0:
                    ...
                    (x_upperleft, y_upperleft, width, height) = faces
                    ...
                    ##-your-code-starts-here-##
                    p0_test = [[x + x_upperleft, y + y_upperleft] for x, y in p0]
                    p0_test = np.array(p0_test)
                    for i in range(len(p0)):
                        p0[i][0] = p0[i][0] + x_upperleft
                        p0[i][1] = p0[i][1] + y_upperleft
                    ##-your-code-ends-here-##
                ...
                good_pts = p1[isFound == 1].astype(np.int32)
                ##-your-code-starts-here-##
                ...
                for point in good_pts:
                    x = point[0]
                    y = point[1]
                    cv2.drawMarker(img=img, position=(x, y), color=(0, 0, 255), markerSize=5, thickness=2)
                ##-your-code-ends-here-##

                # Update p0 (which points should be kept?) and gray_prev for
                # next iteration
                ##-your-code-starts-here-##
                p0 = good_pts.astype(np.float32)
                gray_prev = gray
                ##-your-code-ends-here-##
